...
Story 直接在紧凑侧栏编辑，描述适应文字高度，次要字段折叠为“更多属性”。描述下方的 + 支持导入电脑文件或选择库内文件，附件显示为缩略图和文件名，移除关联不会删除原文件。Task / Activity 使用右侧窄面板，正文仅在编辑时显示格式工具；附件显示为缩略图或文件名。添加 Task 使用独立列，支持自定义任务颜色。
...
右键文件夹 → 新建故事地图，可选择轻量模板或完整示例。每张地图保存为独立的 `.storymap` 文件（JSON 内容），旧地图继续兼容。新增活动、任务、故事和里程碑使用弹窗；点击里程碑可编辑，含故事的里程碑不能删除。关联笔记支持搜索、选择和解除关联。
